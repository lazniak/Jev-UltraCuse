...
| bramka w kodzie | progi z `consts`, zgodność `goal_reached` ∧ `op=done`, lista nieodwracalnych + `is_destructive ≥ 0.5` → `--allow-irreversible` i ≥ 0.85 (click, key i type), kontrolki `disabled` odrzucane; wstrzyknięcie tylko w uchwyt okna skanowany w tym kroku (tuż przed `SendInput`); przemieszczenie przez użytkownika = przegląd okien, nie wejście (ADR-004) | 1.6 (IsPassword), 2.4 (potwierdzenie głosem) |
...
| System Two | `uc-two` (2026-09-21): plan → podcele, ratunek jako ostatni szczebel drabiny, tekst na żądanie; modal wyboru modelu; bramki `from_advice` | pomiar R5 (TASKS 3.7) |
| niepewność | drabina ADR-004: kontekst → przegląd okien (`Switch`/`ShowDesktop`) → System Two → `Uncertain`; miejsce pracy podąża za akcjami pętli, bez wskazywania okna | pomiar R6 (TASKS 1.13) |
